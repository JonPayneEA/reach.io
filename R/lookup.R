...
find_stations <- function(names     = NULL,
  ...
  # Internal helper — fetches stations for a single query and trims to
  # the columns we care about, silently dropping any that are absent in
  # a given response (e.g. riverName is not always populated)
  fetch_stations <- function(query) {
    ...
    dt[, intersect(keep_cols, names(dt)), with = FALSE]
  }
  ...
  # De-duplicate by notation in case the same station was matched by
  # more than one lookup method
  unique(data.table::rbindlist(results, fill = TRUE), by = "notation")
}
